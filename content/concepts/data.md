...
例えば、独裁者の配分額は `num_allocation` や `allocation_amount`、もしくは単に `x` というカラム名を割り当てても良いでしょう。
...
class Player(BasePlayer):
    num_allocation = models.IntegerField()
```

このように定義すると、`Player` テーブルに `num_allocation` という数値型のカラムを定義することができます。
